...
def mnist():
    ...
    for im, label in zip(torch.load("data/corruptmnist/train_images_0.pt"), torch.load("data/corruptmnist/train_target_0.pt")):
        trainset.append((im[None, :, :], label.item()))
    train = torch.utils.data.DataLoader(trainset, batch_size=64, shuffle=True)

    for im, label in zip(torch.load("data/corruptmnist/test_images.pt"), torch.load("data/corruptmnist/test_target.pt")):
